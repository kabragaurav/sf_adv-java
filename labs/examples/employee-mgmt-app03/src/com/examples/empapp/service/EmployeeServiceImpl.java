...
//		Collections.sort(empList, EMPLOYEE_NAME_ASC_SORT);
    ...
    public List<Integer> getEmployeeIdsAgeGreaterThan(int age) {
        List<Integer> empIds = employees.values().stream().filter(emp -> emp.getAge() > age).map(emp -> emp.getEmpId()).collect(Collectors.toList());
        return empIds;
    }
    ...
    public Map<String, Long> getEmployeeCountByDepartment() {
        ...
        return employees.values().stream().map(Employee::getDepartment) // output -> Department name
    ...
    public Map<String, Long> getEmployeeCountByDepartmentOdered() {
        return employees.values().stream().sorted(Comparator.comparing(Employee::getDepartment)).collect(Collectors.groupingBy(Employee::getDepartment, LinkedHashMap::new, Collectors.counting()));
    }

    // Get Department wise average Employee age ordered by Department name
    public Map<String, Double> getAvgEmployeeAgeByDept() {
        return employees.values().stream().sorted(Comparator.comparing(Employee::getDepartment)).collect(Collectors.groupingBy(Employee::getDepartment, LinkedHashMap::new, Collectors.averagingInt(Employee::getAge)));
    }
    ...
    public List<String> getDepartmentsHaveEmployeesMoreThan(int criteria) {
        ...
        return employees.values().stream().sorted(Comparator.comparing(Employee::getDepartment)).collect(Collectors.groupingBy(Employee::getDepartment, Collectors.counting()))
    ...
    public List<String> getEmployeeNamesStartsWith(String prefix) {
        return employees.values().stream().filter(emp -> emp.getName().startsWith(prefix)).map(emp -> emp.getName()).collect(Collectors.toList());
    }
...
//				employee.setEmpId(Integer.parseInt(tokenizer.nextToken()));
    ...
    public void bulkExport() {
        try (FileWriter out = new FileWriter("./output/employee-output.txt")) {
            employees.values()
                    .stream()
                    .map(emp -> emp.getEmpId() + "," + emp.getName() + "," + emp.getAge() + "," + emp.getDesignation() + "," + emp.getDepartment() + "," + emp.getCountry() + "\n")
                    .forEach(rec -> {
                        try {
                            out.write(rec);
                        } catch (IOException e) {
                            System.out.println("Error occurred while writing employee data into file. " + e.getMessage());
                            e.printStackTrace();
                        }
                    });
            System.out.format("%d Employees are exported successfully.", employees.values().size());
        } catch (IOException e) {
            System.out.println("Error occurred while exporting employee data. " + e.getMessage());
        }
    }
}
